...
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "gap_rl", "localgrasp")))
import glob
import time
import argparse
import json
...
def compute_metrics(flags_history, heights_history):
    # flags_history: list of [is_robot_static, is_obj_grasp, is_obj_static, is_obj_lift]
    flags = np.array(flags_history, dtype=bool)
    is_robot_static = flags[:, 0]
    is_obj_grasp = flags[:, 1]
    is_obj_static = flags[:, 2]
    is_obj_lift = flags[:, 3]
    
    is_success = is_robot_static & is_obj_grasp & is_obj_static & is_obj_lift
    success_terminal = is_success[-1]
    
    grasp_ever = np.any(is_obj_grasp)
    lift_ever = np.any(is_obj_lift)
    
    grasp_without_lift = grasp_ever and not lift_ever
    slipped_after_lift = lift_ever and not success_terminal
    
    grasp_transitions = np.diff(is_obj_grasp.astype(int))
    grasp_loss = np.any(grasp_transitions < 0)
    
    late_success_timing = -1
    if success_terminal:
        not_success_idx = np.where(~is_success)[0]
        if len(not_success_idx) == 0:
            late_success_timing = 0
        else:
            late_success_timing = not_success_idx[-1] + 1
            
    stable_grasp = is_obj_grasp & is_obj_static
    stable_then_dropped = False
    if np.any(stable_grasp):
        first_stable_idx = np.argmax(stable_grasp)
        if np.any(~is_obj_grasp[first_stable_idx:]):
            stable_then_dropped = True
            
    peak_height = np.max(heights_history)
    
    return {
        "success_terminal": bool(success_terminal),
        "grasp_without_lift": bool(grasp_without_lift),
        "slipped_after_lift": bool(slipped_after_lift),
        "grasp_loss": bool(grasp_loss),
        "late_success_timing": int(late_success_timing),
        "stable_then_dropped": bool(stable_then_dropped),
    ...
    for rseed in args.seeds:
        ...
        for eval_dataset in args.eval_datasets:
            ...
            for gen_traj_mode in args.gen_traj_modes:
                ...
                for model_id in model_ids:
                    ...
                    with torch.no_grad():
                        for num in tqdm(range(args.obj_test_num), desc=f'Processing 2M', colour='red', leave=True):
                            ...
                            ep_metrics = compute_metrics(ep_flags, ep_heights)
                ...
                if all_ep_metrics:
                    agg_metrics = {k: np.mean([ep[k] for ep in all_ep_metrics]) for k in all_ep_metrics[0].keys()}
                else:
                    agg_metrics = {}
                ...
                if all_ep_metrics:
                    csv_path = f"{log_path}/{result_path}/{gen_traj_mode}/failure_metrics.csv"
                    with open(csv_path, "w", newline="") as f:
                        writer = csv.DictWriter(f, fieldnames=all_ep_metrics[0].keys())
                        writer.writeheader()
                        writer.writerows(all_ep_metrics)
